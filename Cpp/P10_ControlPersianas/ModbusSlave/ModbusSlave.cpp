 ...
 *
 *   Configure the Input Bank Register: Registros de entradas digitales
 */
...
void ModbusSlave::set_OutputHoldingRegister(uint16_t hr, uint16_t val){
	ARegs[hr] = val;

	if(writeholdingregister != NULL){
		writeholdingregister(hr);
	}
}
...
/************************************************************************
 * 
 * write_holding_register(slave_id, write_addr, data_array, registers_array)
 * 
 * Write a single int val into a single holding register of the slave. 
 * 
 *************************************************************************/

int ModbusSlave::write_holding_register(unsigned int write_addr, unsigned char *query, uint16_t *regs)
{
        ...
        if(writeholdingregister != NULL)
        	writeholdingregister(write_addr);
...
int ModbusSlave::write_coil(unsigned int write_addr, unsigned char *query, uint16_t *regs){
	...
	//Evento cuando registro de coil ha sido actualizado
	if(writecoil != NULL)
	{
		writecoil(write_addr);
	}
...
int ModbusSlave::read_discrete_registers(unsigned int Func, unsigned int start_addr, unsigned char reg_count, uint16_t *regs)
{
	...
	packet[2] 		= 1;            //Sólo enviaremos un máximo de 16 registros;
...
//		return preset_multiple_registers(start_addr,query[REGS_L],query,A_Regs);
	...
	case FC_WRITE_REG:
		write_holding_register(start_addr,query,ARegs);
		break;

	case FC_WRITE_COIL:
		write_coil(start_addr,query,CRegs);
		break;
	}

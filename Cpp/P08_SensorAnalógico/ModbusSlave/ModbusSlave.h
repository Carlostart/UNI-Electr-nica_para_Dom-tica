...
/***        Type Definitions                                              ***/
/****************************************************************************/
typedef void (*TNotifyModBusWriteRegisterEvent)(unsigned int);
...
class ModbusSlave {
	private:
		...
		int write_regs(unsigned int start_addr, unsigned char *query, uint16_t *regs);
		int validate_request(unsigned char *data, unsigned char length);
		int preset_multiple_registers(unsigned int start_addr,unsigned char count,unsigned char *query, uint16_t *regs);

		int write_holding_register(unsigned int write_addr, unsigned char *query, uint16_t *regs);
		int write_coil(unsigned int write_addr, unsigned char *query, uint16_t *regs);
		void configure(long baud, char parity, char txenpin);
		unsigned long Calcular_T35(long baud);
	...
	public:
		...
		TNotifyModBusWriteRegisterEvent	writeholdingregister;
		TNotifyModBusWriteRegisterEvent	writecoil;
		...
		//int read_holding_registers(unsigned int start_addr, unsigned char reg_count, uint16_t *regs);
		int read_Analog_registers(unsigned char Func, unsigned int start_addr,unsigned char reg_count, uint16_t *regs);
		int read_discrete_registers(unsigned int Func, unsigned int start_addr, unsigned char reg_count, uint16_t *regs);
		...
		void set_BankCoils(uint16_t *Regs, uint16_t lengthRegs);
		void set_BankDiscreteInputs(uint16_t *Regs, uint16_t lengthRegs);
		void set_BankOutputHoldingRegister(uint16_t *Regs, uint16_t lengthRegs);
		void set_BankInputRegister(uint16_t *Regs, uint16_t lengthRegs);

		void set_OutputHoldingRegister(uint16_t hr, uint16_t val);
	};
